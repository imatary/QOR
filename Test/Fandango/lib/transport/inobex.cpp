...
static bool set_sock_opts(socket_t fd)
{
	 ...
	 */
	int v6only = 0;
	(void)setsockopt(fd, IPPROTO_IPV6, IPV6_V6ONLY, (const char*) &v6only, sizeof(v6only));
#endif
	return true;
}

//------------------------------------------------------------------------------
static void map_ip4to6( sockaddr_in* in, sockaddr_in6* out)
{
...
{
	inobex_data* data = (inobex_data*)(self->trans->data);
...
{
	inobex_data* server_data = (inobex_data*)(server->trans->data);
	inobex_data* data = (inobex_data*)(self->trans->data);
...
{
	obex_transport* trans = (obex_transport*)(self->trans);
	inobex_data* data = (inobex_data*)(self->trans->data);
...
//------------------------------------------------------------------------------
obex_transport* inobex_transport_create(void)
{
