...
//------------------------------------------------------------------------------
obex_transport* obex_transport_create(struct obex_transport_ops *ops)
{
	obex_transport* trans = reinterpret_cast< obex_transport* >( calloc(1, sizeof(*trans)) );
...
result_t obex_transport_handle_input(obex_t *self)
{
	...
	if (self->trans->connected && self->obex_msg_rx_status()) 
	{
...
{
	databuffer* msg = self->rx_msg;
	size_t msglen = msg->get_length();
	void *buf;

	if (!self->trans->connected)
		return 0;

	if (msg->set_size( msglen + self->mtu_rx))
		return -1;

	buf = (uint8_t *)msg->get() + msglen;

	if (self->trans->ops->read) 
	{
		ssize_t ret = self->trans->ops->read(self, buf, max);
		if (ret > 0)
			msg->append(nullptr, ret);
		return ret;
	} else
		return 0;
}
...
{
	obex_transport_ops *ops = self->trans->ops;
	int i;
