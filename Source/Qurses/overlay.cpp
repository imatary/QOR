...
// Thanks to Andreas Otte <venn@@uni-paderborn.de> for the corrected overlay()/overwrite() behavior.

//------------------------------------------------------------------------------
static int _copy_win( const WINDOW* src_w, WINDOW* dst_w, int src_tr, int src_tc, int src_br, int src_bc, int dst_tr, int dst_tc, bool overlay )
{
	__QCS_FCONTEXT( "_copy_win" );
...
int overlay( const WINDOW* src_w, WINDOW* dst_w )
{
    ...
    return _copy_win( src_w, dst_w, src_start_y, src_start_x, src_start_y + ydiff, src_start_x + xdiff, dst_start_y, dst_start_x, TRUE );
}
