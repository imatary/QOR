...
syslook(register unsigned char *w, register const struct sysnod syswds[], int n)
{
	...
	while (low <= high)
	{
		...
		if ((cond = cf(w, (const unsigned char*)syswds[mid].sysnam)) < 0)
			high = mid - 1;
		else if (cond > 0)
			low = mid + 1;
		else
...
//--------------------------------------------------------------------------------
void replace( register unsigned char** a, const unsigned char* v )
{
	free( *a );
	*a = make( v );
}
...
//--------------------------------------------------------------------------------
void dfault( namnod* n, const unsigned char* v )
{
	if( n->namval == 0 )
	{
		assign(n, v);
...
//--------------------------------------------------------------------------------
void assign( namnod* n, const unsigned char* v )
{
	if( n->namflg & N_RDONLY )
	{
		failed( n->namid, (const unsigned char*)wtfailed );
	}
	...
	{
		if( n == &pathnod || eq( n->namid, (const unsigned char*)"SHELL" ) )
		{
			failed( n->namid, (const unsigned char*)restricted );
		}
	}
#endif
...
//	extern const char	badargs[];
	...
	if (eq(*names, (const unsigned char*)"-r")) {
		if (*++names == NULL)
			error((const unsigned char*)badargs);
		newwc = readwc;
	} else
		newwc = nextwc;
	...
	while (n)
	{
		assign(n, (const unsigned char*)nullstr);
		if (*names)
			n = lookup(*names++);
		else
			n = 0;
...
lookup(register unsigned char *nam)
{
	...
	if (!chkid(nam))
		failed(nam, (const unsigned char*)notid);
...
printro(register struct namnod *n)
{
	if (n->namflg & N_RDONLY)
	{
		prs_buff((const unsigned char*)readonly);
		prc_buff(SPACE);
		prs_buff(n->namid);
		prc_buff(NL);
	}
...
printexp(register struct namnod *n)
{
	...
	{
		prs_buff((const unsigned char*)_export);
		prc_buff(SPACE);
		prs_buff(n->namid);
		prc_buff(NL);
	}
...
pushnam(register struct namnod *n)
{
	...
	if (namval)
	{
		...
		p = movstrstak((const unsigned char*)"=", p);
...
unset_name(register unsigned char *name)
{
	...
	{
		if (n->namflg & N_RDONLY)
			failed(name, (const unsigned char*)wtfailed);
		...
		if (n == &pathnod ||
		    ...
		    n == &mchknod)
		{
			failed(name, (const unsigned char*)badunset);
		}

#ifndef RES

		if ((flags & rshflg) && eq(name, (const unsigned char*)"SHELL"))
			failed(name, (const unsigned char*)restricted);
...
dolocale(char *nm)
{
	 ...
	 */
	if ((*nm != 'L') ||
	    (!(eq((const unsigned char*)nm, (const unsigned char*)"LC_ALL") || eq((const unsigned char*)nm, (const unsigned char*)"LC_CTYPE") || eq((const unsigned char*)nm, (const unsigned char*)"LANG"))))
		return;
	...
	for (lv = 0, fe = 0; localevar[lv]; lv++) {
		if ((n = findnam((unsigned char*)localevar[lv]))) {
			...
			fake_env[fe++] = p = (char*)alloc(length((const unsigned char*)localevar[lv])
					       + length(n->namval) + 2);
			/* copy name */
			q = localevar[lv];
			while (*q)
