...
int slk_init( int fmt )
{
    ...
    slk = (SLK*)( calloc(labels, sizeof(struct SLK)) );
...
int slk_noutrefresh( void )
{
    __QCS_FCONTEXT( "slk_noutrefresh" );
...
void PDC_slk_initialize( void )
{
	__QCS_FCONTEXT( "PDC_slk_initialize" );
...
int PDC_mouse_in_slk( int y, int x )
{
	__QCS_FCONTEXT( "PDC_mouse_in_slk" );
    int i;

    // If the line on which the mouse was clicked is NOT the last line of the screen, we are not interested in it.
