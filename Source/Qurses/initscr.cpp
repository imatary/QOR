...
const char* _curses_notice = "Qurses QOR Curses 1.0 derived from PDCurses 3.4 - Public Domain 2008";
...
WINDOW* Xinitscr( int argc, char *argv[] )
{
    ...
    // We have to sort out ripped off lines here, and reduce the height of stdscr by the number of lines ripped off 
...
//------------------------------------------------------------------------------
__QOR_INTERFACE( QURSES_QOR ) WINDOW* initscr()
{
	__QCS_FCONTEXT( "initscr" );
...
SCREEN* newterm( const char* type, FILE* outfd, FILE* infd )
{
    __QCS_FCONTEXT( "newterm" );

    return Xinitscr( 0, NULL ) ? SP : NULL;
}

//------------------------------------------------------------------------------
SCREEN* set_term( SCREEN* pnew )
{
    __QCS_FCONTEXT( "set_term" );

    // We only support one screen

    return ( pnew == SP ) ? SP : NULL;
}

//------------------------------------------------------------------------------
void delscreen( SCREEN* sp )
{
	__QCS_FCONTEXT( "delscreen" );
...
//------------------------------------------------------------------------------
__QOR_INTERFACE( QURSES_QOR ) int resize_term( int nlines, int ncols )
{
    __QCS_FCONTEXT( "resize_term" );
    ...
    if( wresize( curscr, SP->lines, SP->cols ) == ERR || wresize( stdscr, LINES, COLS ) == ERR || wresize( pdc_lastscr, SP->lines, SP->cols ) == ERR )
	{
        return ERR;
	}
