 ...
 * @return a socket
 */
struct obex_sock * obex_transport_sock_create(int domain, int proto, socklen_t addr_size, unsigned int flags)
{
	obex_sock* sock = reinterpret_cast< obex_sock* >( calloc(1, sizeof(*sock)) );
...
ssize_t obex_transport_sock_send( obex_sock *sock, databuffer *msg, int64_t timeout)
{
	...
	{
		status = send(fd, (const char*)(msg->get()), size, 0);
	}	
	else/* The following are not really transport errors. */
#if defined(_WIN32)
...
struct obex_sock * obex_transport_sock_accept(struct obex_sock *sock)
{
	...
	if (flags & OBEX_FL_KEEPSERVER)
		client = (obex_sock*)(calloc(1, sizeof(*sock)));
	else
		client = sock;
...
{
	socket_t fd = sock->fd;
	ssize_t status = recv(fd, (char*)buf, buflen, 0);
