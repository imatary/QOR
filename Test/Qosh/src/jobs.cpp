...
struct job
{
	job *j_nxtp;	/* next job in job ID order */
	job *j_curp;	/* next job in job currency order */
	termios j_stty;	/* termio save area when job stops */
...
static termios 	mystty;	 /* default termio settings		 */
static int 		eofflg,
...
static job 	*jobcur, /* active jobs listed in currency order */
			**nextjob,
			*thisjob,
...
str2job(register char *cmd, register char *job, int mustbejob)
{
	...
	if (*job != '%')
		jp = pgid2job(stoi((const unsigned char*)job));
	else if (*++job == 0 || *job == '+' || *job == '%' || *job == '-') {
		jp = jobcur;
		if (*job == '-' && jp)
			jp = jp->j_curp;
	} else if (*job >= '0' && *job <= '9') {
		i = stoi((const unsigned char*)job);
		...
		for (njp = jobcur; njp; njp = njp->j_curp) {
			...
			for (p = njp->j_cmd, j = strlen(p); j >= i; p++, j--) {
				if (strncmp(job, p, i) == 0) {
					if (jp != 0)
						failed((const unsigned char*)cmd, (const unsigned char*)ambiguous);
					jp = njp;
					break;
				}
			}
	...
	if (mustbejob && (jp == 0 || jp->j_jid == 0))
		failed((const unsigned char*)cmd, (const unsigned char*)nosuchjob);
...
int
settgid(pid_t _new, pid_t expected)
{
...
printjob(register struct job *jp, int propts)
{
	...
	if (propts & PR_STAT) {
		...
		if (jp->j_flag & J_SIGNALED) {
			...
			if ((sigstr = str_signal(jp->j_xval)) != NULL) {
				sp -= strlen(sigstr);
				prs_buff((const unsigned char*)sigstr);
			} else {
				itos(jp->j_xval);
				sp -= strlen((const char*)numbuf) + 7;
				prs_buff((const unsigned char*)"Signal ");
				prs_buff(numbuf);
			}
			if (jp->j_flag & J_DUMPED) {
				sp -= strlen(coredump);
				prs_buff((const unsigned char*)coredump);
			}
		} else if (jp->j_flag & J_DONE) {
			itos(jp->j_xval);
			sp -= strlen(exited) + strlen((const char*)numbuf) + 2;
			prs_buff((const unsigned char*)exited);
			prc_buff('(');
			itos(jp->j_xval);
			prs_buff(numbuf);
			prc_buff(')');
			...
			sp -= strlen(running);
			prs_buff((const unsigned char*)running);
		}
		if (sp < 1)
			sp = 1;
	}
	...
	if (propts & PR_CMD) {
		while (sp-- > 0)
			prc_buff(SPACE);
		prs_buff((const unsigned char*)jp->j_cmd);
		sp = 1;
	}
	...
	if (propts & PR_PWD) {
		while (sp-- > 0)
			prc_buff(SPACE);
		prs_buff((const unsigned char*)"(wd: ");
		prs_buff((const unsigned char*)jp->j_pwd);
		prc_buff(')');
	}
		...
		if (check_if & JOB_STOPPED) {
			for (jp = joblst; jp; jp = jp->j_nxtp) {
				if (jp->j_jid && (jp->j_flag & J_STOPPED)) {
					prs((const unsigned char*)jobsstopped);
					prc(NL);
					return (0);
				}
			}
		...
		if (check_if & JOB_RUNNING) {
			for (jp = joblst; jp; jp = jp->j_nxtp) {
				if (jp->j_jid && (jp->j_flag & J_RUNNING)) {
					prs((const unsigned char*)jobsrunning);
					prc(NL);
					return (0);
				}
			}
...
allocjob(register char *cmd, register unsigned char *cwd, int monitor)
{
	...
	cwdlen = strlen((const char*)cwd) + 1;
	...
	strcpy(jp->j_pwd, (const char*)cwd);
...
makejob(int monitor, int fg)
{
	if (monitor) {
		...
		if (!ioset)
			renamef(chkopen((const unsigned char*)devnull, 0), 0);
	}
}
...
sysjobs(int argc, char *argv[])
{
	...
	if ((flags & jcflg) == 0)
		failed((const unsigned char*)cmd, (const unsigned char*)nojc);

	while ((c = getopt(argc, argv, "lpx")) != -1) {
		if (propts) {
			failure((const unsigned char*)usage, (const unsigned char*)jobsuse);
			goto err;
		}
		switch (c) {
			case 'x':
...
sysfgbg(int argc, char *argv[])
{
	...
	fg = eq((const unsigned char*)"fg", (const unsigned char*)cmd);

	if (*++argv == 0) {
		struct job *jp;
		for (jp = jobcur; ; jp = jp->j_curp) {
			if (jp == 0)
				failed((const unsigned char*)cmd, (const unsigned char*)nocurjob);
			if (jp->j_jid)
				break;
		}
		restartjob(jp, fg);
...
sigv(char *cmd, int sig, char *args)
{
	...
	if (*args == '%') {
		...
		do {
			if (*args < '0' || *args > '9') {
				failure((const unsigned char*)cmd, (const unsigned char*)badid);
				return;
			}
			id = (id * 10) + (*args - '0');
		} while (*++args);
	...
	if (sig == SIGSTOP) {
		if (id == mysid || id == mypid && mypgid == mysid) {
			failure((const unsigned char*)cmd, (const unsigned char*)loginsh);
			return;
		}
	...
	if (kill(id, sig) < 0) {
		...
		switch (errno) {
			case EPERM:
				failure((const unsigned char*)cmd, (const unsigned char*)eacces);
				break;

			case EINVAL:
				failure((const unsigned char*)cmd, (const unsigned char*)badsig);
				break;

			default:
				if (pgrp)
					failure((const unsigned char*)cmd, (const unsigned char*)nosuchpgid);
				else
					failure((const unsigned char*)cmd, (const unsigned char*)nosuchpid);
				break;
		}
...
sysstop(int argc, char *argv[])
{
	char *cmd = *argv;
	if (argc <= 1)
		failed((const unsigned char*)usage, (const unsigned char*)stopuse);
	while (*++argv)
...
syskill(int argc, char *argv[])
{
	...
	if (argc == 1) {
		failure((const unsigned char*)usage, (const unsigned char*)killuse);
		return;
	}
	...
	if (argv[1][0] == '-') {
		...
		if (argc == 2) {
			...
			if (!eq((const unsigned char*)argv[1], (const unsigned char*)"-l")) {
				failure((const unsigned char*)usage, (const unsigned char*)killuse);
				return;
			}
			...
			for (i = 1; i < MAXTRAP; i++) {
				...
				if (sep)
					prc_buff(sep);
				prs_buff((const unsigned char*)buf);
...
{
	if (argc != 1)
		failed((const unsigned char*)argv[0], (const unsigned char*)badopt);
