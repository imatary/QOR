...
int wnoutrefresh(WINDOW *win)
{
    ...
    for( i = 0, j = begy; i < win->_maxy; i++, j++ )
    {
        if( win->_firstch[ i ] != _NO_CHANGE )
        {
            ...
            // ignore areas on the outside that are marked as changed, but really aren't
    ...
    for( y = 0; y < SP->lines; y++ )
    {
        ...
        if( clearall || curscr->_firstch[ y ] != _NO_CHANGE )
        {
            ...
            while( first <= last )
            {
                ...
                // build up a run of changed cells; if two runs are separated by a single unchanged cell, ignore the break
