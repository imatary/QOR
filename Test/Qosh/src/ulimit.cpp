...
sysulimit(int argc, char **argv)
{
	...
	if (optind == argc) {
		...
		for (c = 0; c < res; c++) {
			...
			if (res > 1) {
				prs_buff((const unsigned char*)rp->name);
				prc_buff('(');
				prs_buff((const unsigned char*)rp->scale);
				prc_buff(')');
				prc_buff(' ');
			}
			if (soft) {
				if (rlimit.rlim_cur == RLIM_INFINITY) {
					prs_buff((const unsigned char*)"unlimited");
				} else  {
	...
	if (eq((const unsigned char*)argv[optind], (const unsigned char*)"unlimited")) {
		limit = RLIM_INFINITY;
	} else {
		args = argv[optind];
	...
	fail:	failure((const unsigned char*)argv[0], (const unsigned char*)badulimit);
	}
