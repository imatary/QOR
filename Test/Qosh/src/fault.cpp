...
//--------------------------------------------------------------------------------
int handle( int sig, void (*func)( int ) )
{
	int	ret;
	nsPlatform::sigaction act, oact;
...
void stdsigs( void )
{
	...
	{
		err = errno;
		failure( (const unsigned char*)"sigaltstack(2) failed with", (const unsigned char*)strerror( err ) );
	}
...
int systrap( int argc, char** argv )
{
	int sig;

	if( argc == 1 ) 
	{
		/* print out the current action associated with each signal handled by the shell
		 *
		 */
		for( sig = 0; sig < MAXTRAP; sig++ ) 
		{
			...
			{
				prn_buff( sig );
				prs_buff( (const unsigned char*)colon );
	...
	else 
	{
		...
		while( *++argv ) 
		{
			if( str_2_sig( *argv, &sig ) < 0 || sig >= MAXTRAP || sig < MINTRAP || sig == SIGSEGV || sig == SIGALRM) 
			{
				failure((const unsigned char*)cmd, (const unsigned char*)badtrap);
			} 
			else if (noa1) 
			{
				/*
			...
			else if (*a1) 
			{
				...
				if (trapflg[sig] & SIGMOD || sig == 0 ||
					...
					handle(sig, fault);
					trapflg[sig] |= SIGMOD;
					replace(&trapcom[sig], (const unsigned char*)a1);
				}
			} 
...
#ifdef	SIGINT
	if (SIGINT < MAXTRAP)
		sigvals[SIGINT] = fault;
#endif
#ifdef	SIGQUIT
	if (SIGQUIT < MAXTRAP)
		sigvals[SIGQUIT] = fault;
#endif
#ifdef	SIGILL
	if (SIGILL < MAXTRAP)
		sigvals[SIGILL] = done;
...
#ifdef	SIGUSR1
	if (SIGUSR1 < MAXTRAP)
		sigvals[SIGUSR1] = done;
#endif
#ifdef	SIGSEGV
	if (SIGSEGV < MAXTRAP)
		sigvals[SIGSEGV] = (void(*)(int))sigsegv;
#endif
#ifdef	SIGEMT
	if (SIGEMT < MAXTRAP)
		sigvals[SIGEMT] = done;
...
#ifdef	SIGUSR2
	if (SIGUSR2 < MAXTRAP)
		sigvals[SIGUSR2] = done;
#endif
#ifdef	SIGPIPE
	if (SIGPIPE < MAXTRAP)
		sigvals[SIGPIPE] = done;
#endif
#ifdef	SIGALRM
	if (SIGALRM < MAXTRAP)
		sigvals[SIGALRM] = fault;
#endif
#ifdef	SIGTERM
	if (SIGTERM < MAXTRAP)
		sigvals[SIGTERM] = fault;
#endif
#ifdef	SIGSTKFLT
	if (SIGSTKFLT < MAXTRAP)
		sigvals[SIGSTKFLT] = done;
#endif
#ifdef	SIGCHLD
	if (SIGCHLD < MAXTRAP)
		sigvals[SIGCHLD] = 0;
...
#ifdef	SIGXCPU
	if (SIGXCPU < MAXTRAP)
		sigvals[SIGXCPU] = done;
#endif
#ifdef	SIGXFSZ
	if (SIGXFSZ < MAXTRAP)
		sigvals[SIGXFSZ] = done;
#endif
#ifdef	SIGVTALRM
	if (SIGVTALRM < MAXTRAP)
		sigvals[SIGVTALRM] = done;
#endif
#ifdef	SIGPROF
	if (SIGPROF < MAXTRAP)
		sigvals[SIGPROF] = done;
#endif
#ifdef	SIGWINCH
	if (SIGWINCH < MAXTRAP)
		sigvals[SIGWINCH] = 0;
