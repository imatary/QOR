...
#if !defined(QOR_VMD_ELEM_H)
#define QOR_VMD_ELEM_H
...
/** \def QOR_VMD_ELEM(elem,...)
    ...
    Matching Identifiers: If the filter is specified as the identifier type, QOR_VMD_TYPE_IDENTIFIER,
             ...
             they cannot be any of the specific QOR_VMD_ optional parameters in order to be
    ...
    Splitting: Splitting allows the macro to return the rest of the sequence
             ...
             If QOR_VMD_RETURN_AFTER is specified the return is a tuple
             ...
             If QOR_VMD_RETURN_NO_AFTER, the default, is specified no splitting
             occurs. 
     ...
     Return Type: The element accessed can be changed to return both the type
             ...
             If QOR_VMD_RETURN_NO_TYPE, the default, is specified no type is returned 
             as part of the element accessed. 
             
             If QOR_VMD_RETURN_TYPE is specified the specific type of the element
             ...
             the default QOR_VMD_RETURN_NO_TYPE is in effect.
             
     Index:  If the filter is specified as the identifier type, QOR_VMD_TYPE_IDENTIFIER,
             ...
             If QOR_VMD_RETURN_INDEX is specified an index is returned
             as part of the result. 
             
             If QOR_VMD_RETURN_NO_INDEX, the default, is specified
             ...
             If element access fails the index is empty. If there is no QOR_VMD_TYPE_IDENTIFIER 
             filter or if there are no matching identifiers the QOR_VMD_RETURN_INDEX is ignored 
    ...
    returns   = With no optional parameters the element accessed is returned, or emptiness if
                ...
                and not a tuple. Splitting with QOR_VMD_RETURN_AFTER returns a tuple where the element accessed
                is the first tuple element and the rest of the sequence is the second tuple element.
                Splitting with QOR_VMD_RETURN_ONLY_AFTER returns the rest of the sequence after
                ...
                QOR_VMD_TYPE_IDENTIFIER is specified and matching identifiers are specified.
                When the index is returned with QOR_VMD_RETURN_AFTER it is the third element
...
#define QOR_VMD_ELEM(elem,...) \
    QOR_VMD_DETAIL_SEQUENCE_ELEM(QOR_VMD_ALLOW_ALL,elem,__VA_ARGS__) \
/**/

/** \def QOR_VMD_ELEM_D(d,elem,...)
...
#define QOR_VMD_ELEM_D(d,elem,...) \
    QOR_VMD_DETAIL_SEQUENCE_ELEM_D(d,QOR_VMD_ALLOW_ALL,elem,__VA_ARGS__) \
/**/

#endif /* QOR_PP_VARIADICS */
#endif /* QOR_VMD_ELEM_H */
