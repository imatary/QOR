...
{
	btobex_rfcomm_data* data = (btobex_rfcomm_data*)(self->trans->data);
	socklen_t len = sizeof(sockaddr_rc);
	...
	data->sock = obex_transport_sock_create(AF_BLUETOOTH, BTPROTO_RFCOMM, len, self->init_flags);
...
//------------------------------------------------------------------------------
static bool btobex_set_local_addr(obex_t *self, struct sockaddr *addr, size_t len)
{
	btobex_rfcomm_data* data = (btobex_rfcomm_data*)(self->trans->data);
...
//------------------------------------------------------------------------------
static void btobex_addr2sock(const bdaddr_t *addr, uint8_t channel, sockaddr_rc* sock)
{
 ...
 *
 */
void btobex_prepare_connect(obex_t *self, const bdaddr_t *src, const bdaddr_t *dst, uint8_t channel)
{
	btobex_rfcomm_data* data = (btobex_rfcomm_data*)(self->trans->data);
	sockaddr_rc sock;
	...
	obex_transport_sock_set_remote(data->sock, (struct sockaddr *) &sock, sizeof(sock));
}
...
void btobex_prepare_listen(obex_t *self, const bdaddr_t *src, uint8_t channel)
{
	...
	obex_transport_sock_set_local(data->sock, (struct sockaddr *) &sock, sizeof(sock));
}
...
{
	btobex_rfcomm_data* server_data = (btobex_rfcomm_data*)(server->trans->data);
	btobex_rfcomm_data* data = (btobex_rfcomm_data*)(self->trans->data);
