...
static void custom_cleanup (obex_t *self)
{
	...
	obex_ctrans_t* data = (obex_ctrans_t*)(self->trans->data);
...
void custom_set_data(obex_t *self, void *data)
{
	...
	obex_ctrans_t* ctrans = (obex_ctrans_t*)(trans->data);
...
{
	const obex_ctrans_t* old = (const obex_ctrans_t*)(from->trans->data);
	obex_ctrans_t* ctrans = (obex_ctrans_t*)(self->trans->data);
...
static ssize_t custom_write(obex_t *self, buf_t *msg)
{
	...
	return ctrans->write(self, ctrans->customdata, (uint8_t*)(msg->get()), msg->get_length());
}
...
static ssize_t custom_read(obex_t *self, void *buf, int size)
{
	...
	if (ctrans->read) 
	{
		return ctrans->read(self, ctrans->customdata, (uint8_t*)buf, size);
...
{
	obex_transport_ops* ops = (obex_transport_ops*)(calloc(1, sizeof(*ops)));
	obex_transport* trans;
