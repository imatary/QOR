...
int main( int c, char* v[], char* e[] )
{
	...
	//If the first character of the last path element of v[0] is "-" (ex. -sh, or /bin/-sh), this is a login shell	 
	if( v != 0 && ( *simple((unsigned char*)v[0]) == '-') )
	{
		signal( SIGXCPU, SIG_DFL );
		signal( SIGXFSZ, SIG_DFL );

		/* As the previous comment states, this is a login shell. Therefore, we set the login_shell flag to explicitly
	...
	// 'rsflag' is zero if SHELL variable is set in environment and the simple file part of the value. is rsh
	if (n = findnam((unsigned char*)"SHELL"))
	{
		if (eq((const unsigned char*)"rsh", simple(n->namval)))
			rsflag = 0;
	}

	// a shell is also restricted if the simple name of argv(0) is rsh or -rsh in its simple name 

#ifndef RES

	if (c > 0 && (eq((const unsigned char*)"rsh", simple((unsigned char*)*v)) || eq((const unsigned char*)"-rsh", simple((unsigned char*)*v))))
	{
		rflag = 0;
	}
	...
	//return here for shell file execution but not for parenthesis subshells
	...
	/* default internal field separators Do not allow importing of IFS from parent shell.
	 * setup_env() may have set anything from parent shell to IFS. Always set the default ifs to IFS.*/
	assign( &ifsnod, (const unsigned char*)sptbnl );

	dfault( &timeoutnod, (const unsigned char*)"0" );
	timeoutnod.namflg |= N_RDONLY;

	dfault( &mchknod, (const unsigned char*)MAILCHECK );
	mailchk = stoi( mchknod.namval );

	// initialize OPTIND for getopt
	n = lookup( (unsigned char*)"OPTIND" );
	assign( n, (const unsigned char*)"1" );
	...
	if( ( beenhere++ ) == FALSE )	/* ? profile */
	{
		if( ( login_shell == TRUE ) && ( flags & privflg ) == 0 ) 
		{
			...
			if( ( input = pathopen( (const unsigned char*)nullstr, (const unsigned char*)sysprofile ) ) >= 0 )
			{
			...
			if( ( input = pathopen( homenod.namval, (const unsigned char*)profile ) ) >= 0 )
			{
				exfile( rflag );
				flags &= ~ttyflg;
			}
...
static void exfile( BOOL prof )
{
	...
	for(;;)
	{
		...
		if( ( flags & prompt ) && standin->fstak == 0 && !eof )
		{
			...
			if( ( timeout = atol( (const char*)timeoutnod.namval ) ) > 0 )
			{
				alarm( timeout );
			}
	...
	while( *s ) 
	{
		...
		if( *start && stat( (const char *)start, &statb ) >= 0 ) 
		{
			if( statb.st_size && *ptr && statb.st_mtime != *ptr ) 
			{
				...
				{
					prs( (const unsigned char*)mailmsg );
				}
			}
			*ptr = statb.st_mtime;
		} 
...
//--------------------------------------------------------------------------------
void setmail( unsigned char* mailpath )
{
...
void setwidth( void )
{
	unsigned char* name = lookup( (unsigned char*)"LC_ALL" )->namval;
	if (!name || !*name)
	{
		name = lookup((unsigned char*)"LC_CTYPE")->namval;
	}

	if (!name || !*name)
	{
		name = lookup((unsigned char*)"LANG")->namval;
	}
...
void setmode( int prof )
{
	...
	{
		dfault( &ps1nod, (const unsigned char*)( geteuid() ? stdprompt : supprompt ) );
		dfault( &ps2nod, (const unsigned char*)readmsg );
		flags |= ttyflg | prompt;
