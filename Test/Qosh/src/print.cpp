...
prs(const unsigned char *as)
{
	...
	if ((s = (char *)as) != 0) {
		write(output, s, length((const unsigned char*)s) - 1);
	}
}
...
itos(int n)
{
	...
	*--abuf = (unsigned char)'\0';
	...
	strncpy((char*)numbuf, (const char*)abuf, sizeof (numbuf));
}
...
{
	int len = length((const unsigned char *)s) - 1;
	...
	if (buffd != -1 && len >= BUFLEN) {
		...
		movstr((const unsigned char *)s, &bufp[index]);
		index += len;
	}
}
