...
//--------------------------------------------------------------------------------
int execute( trenod* argt, int xflags, int errorflg, int* pf1, int* pf2 )
{
	...
	if( ( t = argt ) && execbrk == 0 ) 
	{
		...
		case TFND:
			{
				...
				if( flags & rshflg && ( n == &pathnod || eq( n->namid, (const unsigned char*)"SHELL" ) ) )
				{
		...
		case TCOM:
			{
				...
				if( argn && ( flags & noexec ) == 0 )
				{
					...
					if( comtype == NOTFOUND )
					{
						...
						if( pos == 1 )
						{
							failure( *com, (const unsigned char*)notfound );
						}
						else if( pos == 2 )
						{
							failure( *com, (const unsigned char*)badexec );
						}
						else
						{
							failure( *com, (const unsigned char*)badperm );
						}
						break;
					}
					...
					else if( comtype == FUNCTION )
					{
						...
						olddolh = (dolnod *)savargs( funcnt );
						funcnt++;
						index = initio( io, 1 );
						setargs( com );
						execute( (trenod *)(n->namenv), xflags, errorflg, pf1, pf2 );
						execbrk = 0;
						restore( index );
		...
		case TFORK:
		{
			...
			if( !( xflags & XEC_EXECED ) || treeflgs&( FPOU | FAMP ) )
			{
				...
				if( !( treeflgs & FPOU ) )
				{
					monitor = ( !( xflags & XEC_NOSTOP ) && ( flags & ( monitorflg | jcflg | jcoff ) ) == ( monitorflg | jcflg ) );
					if (monitor) 
					{
						int savefd;
				...
				while( ( parent = fork() ) == -1 )
				{
					...
					if( ( forkcnt = ( forkcnt * 2 ) ) > FORKLIM )
					{
						...
						case ENOMEM:
							deallocjob();
							error( (const unsigned char*)noswap );
							break;
						default:
							deallocjob();
							error( (const unsigned char*)nofork );
							break;
						}
					} 
					else if( errno == EPERM ) 
					{
						deallocjob();
						error( (const unsigned char*)eacces );
						break;
					}
					sigchk();
					sleep( forkcnt );
...
execprint(unsigned char **com)
{
	...
	prs((const unsigned char*)execpmsg);
