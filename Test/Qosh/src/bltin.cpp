...
//------------------------------------------------------------------------------
void builtin( int type, int argc, unsigned char** argv, trenod* t )
{
	...
	case SYSDOT:
		if (a1)
		{
			...
			if ((f = pathopen(getpath(a1), a1)) < 0)
				failed(a1, (const unsigned char*)notfound);
			else
				execexp(0, f);
		}
		break;
	...
	case SYSNEWGRP:
		if (flags & rshflg)
			failed(argv[0], (const unsigned char*)restricted);
	...
	case SYSCD:
		...
		else if ((a1 && *a1) || (a1 == 0 && (a1 = homenod.namval)))
		{
...
#ifdef	EMULTIHOP
						case EMULTIHOP:
							failed(a1, (const unsigned char*)emultihop);
							break;
#endif
						case ENOTDIR:
							failed(a1, (const unsigned char*)enotdir);
							break;
						case ENOENT:
							failed(a1, (const unsigned char*)enoent);
							break;
						case EACCES:
							failed(a1, (const unsigned char*)eacces);
							break;
#ifdef	ENOLINK
						case ENOLINK:
							failed(a1, (const unsigned char*)enolink);
							break;
#endif
						default: 
						failed(a1, (const unsigned char*)baddir);
						break;
						}
			}
			else 
			{
				cwd(curstak());
				if (cf((const unsigned char*)nullstr, dir) &&
				    *dir != ':' &&
					any('/', curstak()) &&
					flags & prompt)
				{
		...
		{
			if (a1)
				error((const unsigned char*)nulldir);
			else
				error((const unsigned char*)nohome);
		}
	...
	case SYSSHFT:
		{
			...
			{
				dolc = 0;
				error((const unsigned char*)badshift);
			}
			else
				dolv += places;
		}			
	...
	case SYSREAD:
		if(argc < 2)
			failed(argv[0],(const unsigned char*)mssgargn);
		rwait = 1;
			...
			if (a1)
			{
				while (*++argv)
				{
					...
					if (n->namflg & N_FUNCTN)
						error((const unsigned char*)badexport);
					else
						attrib(n, N_EXPORT);
				}
			}
		...
		if (a1)
		{
			if (a1[0] == '-')
			{
				if (a1[1] == 'r')
					...
					error((const unsigned char*)badopt);
			}
			else
			{
				while (*++argv)
				{
					if (hashtype(hash_cmd(*argv)) == NOTFOUND)
						failed(*argv, (const unsigned char*)notfound);
				}
			}
		}
		else
	...
	case SYSRETURN:
		if (funcnt == 0)
			error((const unsigned char*)badreturn);
	...
	case SYSGETOPT: {
		...
		unsigned char c[2];
		if(argc < 3) {
			failure(argv[0],(const unsigned char*)mssgargn);
			break;
		}
		exitval = 0;
		...
		assign(n, (const unsigned char*)optarg);
		}
		break;

	default:
		prs_buff((const unsigned char*)"unknown builtin\n");
	}
