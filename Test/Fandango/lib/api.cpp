 ...
 */
LIB_SYMBOL obex_t* __stdcall OBEX_Init(int transport, obex_event_t eventcb, unsigned int flags)
{
	obex_t* self;
...
/**
	...
	a custom transport.
 */
LIB_SYMBOL int __stdcall OBEX_RegisterCTransport(obex_t *self, obex_ctrans_t *ctrans)
{
...
/**
	...
	#OBEX_FreeInterfaces).
 */
LIB_SYMBOL void __stdcall OBEX_Cleanup(obex_t *self)
{
 ...
 */
LIB_SYMBOL void __stdcall OBEX_SetUserData(obex_t *self, void *data)
{
	obex_return_if_fail(self != NULL);
	self->userdata=data;
}
 ...
 */
LIB_SYMBOL void* __stdcall OBEX_GetUserData(obex_t *self)
{
...
/**
	...
	callback (optional)
 */
LIB_SYMBOL void __stdcall OBEX_SetUserCallBack(obex_t *self, obex_event_t eventcb, void *data)
{
	obex_return_if_fail(self != NULL);
 ...
 */
LIB_SYMBOL int __stdcall OBEX_SetTransportMTU(obex_t *self, uint16_t mtu_rx,
							uint16_t mtu_tx_max)
{
	...
	return self->set_mtu(mtu_rx, mtu_tx_max);
}
 ...
 */
LIB_SYMBOL int __stdcall OBEX_ServerRegister(obex_t *self, struct sockaddr *saddr, int addrlen)
{
	DEBUG(3, "\n");
 ...
 */
LIB_SYMBOL obex_t* __stdcall OBEX_ServerAccept(obex_t *server, obex_event_t eventcb, void *data)
{
	obex_t *self;
	...
	self = new obex(eventcb, server->init_flags);
	if (self == nullptr)
	{
		return nullptr;
	}
	...
	if (self->set_mtu(server->mtu_rx, server->mtu_tx_max))
	{
		goto out_err;
	}
 ...
 */
LIB_SYMBOL void __stdcall OBEX_SetTimeout(obex_t *self, int64_t timeout)
{
	obex_return_if_fail(self != NULL);
...
/**
	...
	finished.
 */
LIB_SYMBOL int __stdcall OBEX_Work(obex_t *self)
{
	DEBUG(4, "\n");
	obex_return_val_if_fail(self != NULL, -1);
	return self->work();
 ...
 */
LIB_SYMBOL enum obex_data_direction __stdcall OBEX_GetDataDirection(obex_t *self)
{
	DEBUG(4, "\n");
	return self->get_data_direction();
}
 ...
 */
LIB_SYMBOL int __stdcall OBEX_HandleInput(obex_t *self, int timeout)
{
	int result;
	int64_t oldTimeout;
	obex_data_direction dir;
...
/**
	...
	\return -1 on error
 */
LIB_SYMBOL int __stdcall OBEX_CustomDataFeed(obex_t *self, uint8_t *inputbuf, int actual)
{
	DEBUG(3, "\n");
	...
	if (inputbuf && actual > 0)
		self->rx_msg->append(inputbuf, (size_t)actual);

	return self->data_indication();
}
...
/**
	...
	\return -1 on error
 */
LIB_SYMBOL int __stdcall OBEX_TransportConnect(obex_t *self, sockaddr *saddr, int addrlen)
{
	DEBUG(4, "\n");
...
/**
	...
	\return -1 on error
 */
LIB_SYMBOL int __stdcall OBEX_TransportDisconnect(obex_t *self)
{
	DEBUG(4, "\n");
	obex_return_val_if_fail(self != NULL, -1);
 ...
 */
LIB_SYMBOL int __stdcall OBEX_GetFD(obex_t *self)
{
	obex_return_val_if_fail(self != NULL, -1);
 ...
 */
LIB_SYMBOL int __stdcall OBEX_Request(obex_t *self, CObexObject* object)
{
	result_t result;
	...
	object->SetResponseMode( self->rsp_mode );
	self->object = object;
...
/**
	...
	\return -1 on error
 */
LIB_SYMBOL int __stdcall OBEX_CancelRequest(obex_t *self, int nice)
{
	obex_return_val_if_fail(self != NULL, -1);
	return self->cancelrequest(nice);
}
...
/**
	...
	\return -1 on error
 */
LIB_SYMBOL int __stdcall OBEX_ResumeRequest(obex_t *self)
{
 ...
 */
LIB_SYMBOL void __stdcall OBEX_SetReponseMode(obex_t *self, enum obex_rsp_mode rsp_mode)
{
	switch (rsp_mode) {
	...
	case OBEX_RSP_MODE_SINGLE:
		...
		if (self->object)
		{
			self->object->SetResponseMode(rsp_mode);
		}
		break;
 ...
 */
LIB_SYMBOL CObexObject* __stdcall OBEX_ObjectNew(obex_t *self, uint8_t cmd)
{
	CObexObject* object;

	obex_return_val_if_fail(self != NULL, NULL);

	object = new CObexObject();
	if (object == nullptr)
	{
		return nullptr;
	}

	object->setcmd((obex_cmd)cmd);
	/* Need some special woodoo magic on connect-frame */
	if (cmd == OBEX_CMD_CONNECT) 
	{
		if (self->insert_connectframe(object) < 0) 
		{
			delete object;
			object = nullptr;
		}
 ...
 */
LIB_SYMBOL int __stdcall OBEX_ObjectDelete(obex_t *self, CObexObject* object)
{
 ...
 */
LIB_SYMBOL int __stdcall OBEX_ObjectGetSpace(obex_t *self, CObexObject* object,
				unsigned int flags)
{
	obex_return_val_if_fail(self != NULL, -1);
	obex_return_val_if_fail(object != NULL, -1);
	return self->obex_msg_getspace(object, flags);
}
 ...
 */
LIB_SYMBOL int __stdcall OBEX_ObjectAddHeader(obex_t *self, CObexObject* object, uint8_t hi, obex_headerdata_t hv, uint32_t hv_size, unsigned int flags)
{
	obex_return_val_if_fail(self != NULL, -1);
	obex_return_val_if_fail(object != NULL, -1);
	return object->addheader(self, object, hi, hv, hv_size, flags);
}
 ...
 */
LIB_SYMBOL int __stdcall OBEX_ObjectGetNextHeader(obex_t *self, CObexObject* object, uint8_t *hi, obex_headerdata_t *hv, uint32_t *hv_size)
{
	obex_return_val_if_fail(object != NULL, -1);
	return object->getnextheader(hi, hv, hv_size);
}
...
/**
	...
	header.
 */
LIB_SYMBOL int __stdcall OBEX_ObjectReParseHeaders(obex_t *self, CObexObject* object)
{
	obex_return_val_if_fail(object != NULL, -1);
	return object->reparseheaders();
}
 ...
 */
LIB_SYMBOL int __stdcall OBEX_ObjectReadStream(obex_t *self, CObexObject* object, const uint8_t **buf)
{
	size_t size = 0;
	...
	if (buf == NULL) 
	{
		IObexBody* b = new CObexBodyStream(self);
		int result = object->set_body_receiver( b );
	...
	if (buf)
	{
		*buf = (const uint8_t*)( object->read_body( &size ) );
	}
	return (int)size;
}
...
/**
	...
	\return -1 on error
 */
LIB_SYMBOL int __stdcall OBEX_ObjectSetRsp(CObexObject* object, uint8_t rsp, uint8_t lastrsp)
{
	obex_return_val_if_fail(object != NULL, -1);
	return object->setrsp((obex_rsp)rsp, (obex_rsp)lastrsp);
}
 ...
 */
LIB_SYMBOL int __stdcall OBEX_ObjectGetNonHdrData(CObexObject* object, uint8_t **buffer)
{
	obex_return_val_if_fail(object != NULL, -1);
	if (!object->GetRXNonHeaderData())
	{
		return 0;
	}

	*buffer = (uint8_t*)(object->GetRXNonHeaderData()->get());
	return object->GetRXNonHeaderData()->get_length();
}
 ...
 */
LIB_SYMBOL int __stdcall OBEX_ObjectSetNonHdrData(CObexObject* object, const uint8_t *buffer, unsigned int len)
{
	...
	if (object->GetTXNonHeaderData())
	{
		return -1;
	}

	object->SetTXNonHeaderData( new membuf(len) );
	if (object->GetTXNonHeaderData() == nullptr)
	{
		return -1;
	}
	object->GetTXNonHeaderData()->append(buffer, len);
 ...
 */
LIB_SYMBOL int __stdcall OBEX_ObjectSetHdrOffset(CObexObject* object, unsigned int offset)
{
	obex_return_val_if_fail(object != NULL, -1);
	object->SetHeaderOffset( offset );
	return 1;
}
 ...
 */
LIB_SYMBOL int __stdcall OBEX_ObjectGetCommand(obex_t *self, CObexObject* object)
{
...
/**
	...
	be returned.
 */
LIB_SYMBOL char* __stdcall OBEX_ResponseToString(int rsp)
{
	DEBUG(4, "\n");
...
/**
	...
	- Jean II
 */
LIB_SYMBOL int __stdcall OBEX_SetCustomData(obex_t *self, void *data)
{
	obex_return_val_if_fail(self == NULL, -1);
 ...
 */
LIB_SYMBOL void* __stdcall OBEX_GetCustomData(obex_t *self)
{
	obex_return_val_if_fail(self == NULL, NULL);
 ...
 */
LIB_SYMBOL int __stdcall TcpOBEX_ServerRegister(obex_t *self, struct sockaddr *addr, int addrlen)
{
	DEBUG(3, "\n");
...
/**
	...
	It supports IPv4 (AF_INET) and IPv6 (AF_INET6).
 */
LIB_SYMBOL int __stdcall TcpOBEX_TransportConnect(obex_t *self, struct sockaddr *addr, int addrlen)
{
	DEBUG(4, "\n");
 ...
 */
LIB_SYMBOL int __stdcall IrOBEX_ServerRegister(obex_t *self, const char *service)
{
	DEBUG(3, "\n");
 ...
 */
LIB_SYMBOL int __stdcall IrOBEX_TransportConnect(obex_t *self, const char *service)
{
 ...
 */
LIB_SYMBOL int __stdcall BtOBEX_ServerRegister(obex_t *self, const bt_addr_t *src, uint8_t channel)
{
	DEBUG(3, "\n");
 ...
 */
LIB_SYMBOL int __stdcall BtOBEX_TransportConnect(obex_t *self, const bt_addr_t *src, const bt_addr_t *dst, uint8_t channel)
{
	DEBUG(4, "\n");
...
/*
	...
	\return -1 or negative error code on error
 */
LIB_SYMBOL int __stdcall FdOBEX_TransportSetup(obex_t *self, int rfd, int wfd, int mtu)
{
	DEBUG(4, "\n");
 ...
 */
LIB_SYMBOL int __stdcall OBEX_InterfaceConnect(obex_t *self, obex_interface_t *intf)
{
	DEBUG(4, "\n");
 ...
 */
LIB_SYMBOL int __stdcall OBEX_EnumerateInterfaces(obex_t *self)
{
	DEBUG(4, "\n");
 ...
 */
LIB_SYMBOL obex_interface_t* __stdcall OBEX_GetInterfaceByIndex(obex_t *self, int i)
{
	DEBUG(4, "\n");
 ...
 */
LIB_SYMBOL void __stdcall OBEX_FreeInterfaces(obex_t *self)
{
	obex_return_if_fail(self != NULL);
