...
int wgetch( WINDOW* win )
{
    ...
    if( SP->delaytenths )
	{
        ...
        if( win->_delayms )
        {
            // Can't really do millisecond intervals, so delay in 1/20ths of a second (50ms)
    ...
    // refresh window when wgetch is called if there have been changes to it and it is not a pad
    ...
    for( ;; )            // loop for any buffering
    {
        ...
        if( SP->key_code )
        {
            ...
            // filter mouse events; translate mouse clicks in the slk area to function keys
...
unsigned long PDC_get_key_modifiers( void )
{
    __QCS_FCONTEXT( "PDC_get_key_modifiers" );
...
int PDC_save_key_modifiers( bool flag )
{
    __QCS_FCONTEXT( "PDC_save_key_modifiers" );
...
int PDC_return_key_modifiers( bool flag )
{
    __QCS_FCONTEXT( "PDC_return_key_modifiers" );
...
int wget_wch( WINDOW* win, wint_t* wch )
{
    ...
    *wch = key;
    ...
    return SP->key_code ? KEY_CODE_YES : 0;
}

//------------------------------------------------------------------------------
int get_wch( wint_t* wch )
{
    __QCS_FCONTEXT( "get_wch" );
