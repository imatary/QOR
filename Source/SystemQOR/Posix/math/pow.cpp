...
{
    //------------------------------------------------------------------------------
    float Cmath::powf( float x, float y ) // wrapper powf
    {
        float z;
        ...
        if( isnanf( x ) )
        {
            if( y == (float)0.0 )
            {
                ...
                exc.arg2 = (double)y;
                exc.retval = x;
                if( m_fdlib_version == _IEEE_ || m_fdlib_version == _POSIX_ )
                {
                    exc.retval = 1.0;
                }
                else if( !matherr( &exc ) )
        ...
        if( !finitef( z ) )
        {
            if( finitef( x ) && finitef( y ) )
            {
                if( isnanf( z ) )
                {
                    ...
                    {
                        exc.retval = infinity(); // X/Open allow NaN
                    }
    ...
    //------------------------------------------------------------------------------
    double Cmath::pow( double x, double y )
    {
...
#	endif
    ...
    //------------------------------------------------------------------------------
    double Cmath::__ieee754_pow( double x, double y )
    {
        ...
        Cmp_signed__int32 i, j, k, yisint, n;
        ...
        */
        ...
        if( ( ( ( (Cmp_unsigned__int32)hx >> 31 ) - 1 ) | yisint ) == 0 )
        {
            return ( x - x ) / ( x - x );
        }
        ...
        if( ( ( ( (Cmp_unsigned__int32) hx >> 31 ) - 1 ) | ( yisint - 1 ) ) == 0 )
        {
            s = -one;// (-ve)**(odd int)
        }
    ...
    //------------------------------------------------------------------------------
    float Cmath::__ieee754_powf( float x, float y )
    {
        ...
        Cmp_signed__int32 i, j, k, yisint, n;
        Cmp_signed__int32 hx, hy, ix, iy, is;
