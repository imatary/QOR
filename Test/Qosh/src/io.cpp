...
chkpipe(int *pv)
{
	if (pipe(pv) < 0 || pv[INPIPE] < 0 || pv[OTPIPE] < 0)
		error((const unsigned char*)piperr);
}
...
chkopen(const unsigned char *idf, int mode)
{
	...
	if ((rc = open((const char *)idf, mode, 0666)) < 0)
		failed(idf, (const unsigned char*)badopen);
	return (rc);
}
...
create(unsigned char *s)
{
	...
	if ((rc = creat((char *)s, 0666)) < 0)
		failed(s, (const unsigned char*)badcreate);
	return (rc);
}
...
tmpfil(struct tempblk *tb)
{
	...
	if (fd == -1)
		failed(tmpout, (const unsigned char*)badcreate);
	pushtemp(fd, tb);
	return (fd);
}
