	...
	while (iop) {
		...
		if (*ion && (flags&noexec) == 0) {
			...
			if (iof & IODOC) {
				...
				if (eq((const unsigned char*)minus, ion)) {
					fd = -1;
					close(ioufd);
				} else if ((fd = stoi(ion)) >= USERIO) {
					failed(ion, (const unsigned char*)badfile);
				}
				else
					fd = dup(fd);
				...
				failed(ion, (const unsigned char*)restricted);
			} else if (iof & IOAPP &&
...
getpath(unsigned char *s)
{
	...
	{
		if (flags & rshflg)
			failed(s, (const unsigned char*)restricted);
		else
...
execs(unsigned char *ap, register unsigned char *t[])
{
	...
	case ENOMEM:
		failed(p, (const unsigned char*)toobig);

	case E2BIG:
		failed(p, (const unsigned char*)arglist);

	case ETXTBSY:
		failed(p, (const unsigned char*)txtbsy);

#ifdef	ELIBACC
	case ELIBACC:
		failed(p, (const unsigned char*)libacc);
#endif

#ifdef	ELIBBAD
	case ELIBBAD:
		failed(p, (const unsigned char*)libbad);
#endif

#ifdef	ELIBSCN
	case ELIBSCN:
		failed(p, (const unsigned char*)libscn);
#endif

#ifdef	ELIBMAX
	case ELIBMAX:
		failed(p, (const unsigned char*)libmax);
#endif
