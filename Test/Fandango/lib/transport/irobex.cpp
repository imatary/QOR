...
{
	irobex_data* data = (irobex_data*)(self->trans->data);
	socklen_t len = sizeof(struct sockaddr_irda);

	if (data == NULL)
		return false;

	data->sock = obex_transport_sock_create(AF_IRDA, 0, len, self->init_flags);
	if (data->sock == NULL) {
		free(data);
		return false;
	}
 ...
 *
 */
static bool irobex_no_addr( sockaddr_irda *addr)
{
#ifndef _WIN32
...
static bool irobex_query_ias(obex_t *self, uint32_t addr, const char* class_name)
{
	...
	if (fd == INVALID_SOCKET)
		return false;
	err = getsockopt(fd, SOL_IRLMP, IRLMP_IAS_QUERY, (char*)&ias_query, &len);
	close_socket(fd);
...
{
	irobex_data* server_data = (irobex_data*)(server->trans->data);
	irobex_data* data = (irobex_data*)(self->trans->data);
...
static int irobex_find_interfaces(obex_t *self, obex_interface_t **interfaces)
{
	...
	*interfaces = (obex_interface_t*)(calloc(count, sizeof(**interfaces)));
	...
	for (i = 0; i < count; ++i) {
		...
		intf->info = (char*)(calloc(sizeof(dev[i].irdaDeviceName)+2, 1));
		if (intf->info)
...
static ssize_t irobex_write(obex_t *self, databuffer *msg)
{
