...
hashpr(void)
{
	prs_buff((const unsigned char*)"hits	cost	command\n");
	hscan(hashout);
}
...
what_is_path(register unsigned char *name)
{
	...
	if (h)
	{
		...
		{
			case BUILTIN:
				prs_buff((const unsigned char*)" is a shell builtin\n");
				return (0);
			...
			case FUNCTION:
			{
				...
				prs_buff((const unsigned char*)" is a function\n");
				prs_buff(name);
			...
			case REL_COMMAND:
			{
				...
				if ((h->data & DOT_COMMAND) == DOT_COMMAND)
				{
					...
					{
						prs_buff((const unsigned char*)" not found\n");
						return (1);
					}
					else
			...
			case COMMAND:					
				prs_buff((const unsigned char*)" is hashed (");
	...
	{
		prs_buff((const unsigned char*)" is ");
		pr_path(name, cnt);
		prc_buff(NL);
		return (0);
	}
		...
		if (letter(*s))		
		{
			...
			if (*s == '=')
			{
				*s = 0;
				...
				if (eq(start, (const unsigned char*)pathname))
				{
					*s = '=';
					return(1);
				}
