...
prf(register struct trenod *t)
{
	...
	if (t)
	{
		...
		switch(type)
		{
			...
			case TFOR:
				{
					...
					prs_buff((const unsigned char*)"for ");
					prs_buff(f->fornam);

					if (f->forlst)
					{
						arg = f->forlst->comarg;
						prs_buff((const unsigned char*)" in");
					...
					prendlst();
					prs_buff((const unsigned char*)"do");
					prbgnlst();
					prf(f->fortre);
					prendlst();
					prs_buff((const unsigned char*)"done");
				}
				break;
			...
			case TUN:
				if (type == TWH)
					prs_buff((const unsigned char*)"while ");
				else
					prs_buff((const unsigned char*)"until ");
				prf(whptr(t)->whtre);
				prendlst();
				prs_buff((const unsigned char*)"do");
				prbgnlst();
			...
			case TIF:
			{
				...
				prs_buff((const unsigned char*)"if ");
				prf(f->iftre);
				prendlst();
				prs_buff((const unsigned char*)"then");
				prendlst();
				prf(f->thtre);
				...
				{
					prendlst();
					prs_buff((const unsigned char*)"else");
					prendlst();
					prf(f->eltre);
				}

				prendlst();
				prs_buff((const unsigned char*)"fi");
				break;
			}
			...
			case TSW:
				{
					...
					prs_buff((const unsigned char*)"case ");
