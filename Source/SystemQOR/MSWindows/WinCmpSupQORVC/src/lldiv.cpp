...
//To generate assembler function for lldiv

//------------------------------------------------------------------------------
class CJITalldiv : public nsArch::CJITFunctor0< int >
{
public:

	//------------------------------------------------------------------------------
	CJITalldiv( nsArch::CHighLevelAssemblerBase* pHLA, byte* pLaunchPad = 0 ) : CJITFunctor0( pHLA, pLaunchPad )
	{
		if( m_pLaunchPad != 0 )
		{
			Generate();
...
protected:
	...
	virtual FP Generate()
	{
		...
		nsArch::nsx86::CMem Dividend( CPU.reg_esp(), 16 );
		nsArch::nsx86::CMem HiWordDividend( CPU.reg_esp(), 20 );
		nsArch::nsx86::CMem Divisor( CPU.reg_esp(), 24 );
		nsArch::nsx86::CMem HiWordDivisor( CPU.reg_esp(), 28 );
...
;*/
		...
		CPU.mov(CPU.reg_edx(), CPU.reg_ebx());							//edx:eax <-quotient
		//jmp     short L4        ; set sign, restore stack and return
		CPU.short_jmp(Label4);											//set sign, restore stack and return
		/*        
;
...
;
		*/
		...
		CPU.xor_(CPU.reg_edx(), CPU.reg_edx());							//edx:eax <- quotient
		//mov     eax,esi
...
__declspec( allocate( ".jit" ) ) byte _alldiv[ 16 ];

#pragma comment( linker, "/SECTION:.jit,ERW" )

__QCMP_ENDLINKAGE_C

CJITalldiv alldivJIT( &TheMachine()->HLAssembler(), _alldiv );
