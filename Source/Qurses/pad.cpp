...
WINDOW* subpad( WINDOW* orig, int nlines, int ncols, int begy, int begx )
{
    ...
    if( ( begy < orig->_begy ) || ( begx < orig->_begx ) || ( begy + nlines ) > ( orig->_begy + orig->_maxy ) || ( begx + ncols )  > ( orig->_begx + orig->_maxx ) )
	{
        return (WINDOW*)NULL;
	}
